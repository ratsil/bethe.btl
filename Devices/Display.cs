...
namespace BTL
{
	public class Display : Device
	{
		...
		private Display(GraphicsAdapter cDevice)
		{
			...
			{
				_cDevice = cDevice;
				stArea = new Area(0, 0, 720, 576); //UNDONE
				...
				try
				{
					while (Preferences.nQueueDeviceLength > _cXNARender._aqDisplayQueue.Count)
					{
						...
						lock (_cXNARender._aqDisplayQueue)
						{
							...
							_cXNARender._aqDisplayQueue.Enqueue(_cXNARender.TextureCreate(cFrameVideo.aFrameBytes, 720, 576)); //UNDONE
					...
					if (!nStarted && _cXNARender._aqDisplayQueue.Count > 0)
						nStarted = true;
					if (nStarted && _cXNARender._aqDisplayQueue.Count == 0)
						(new Logger()).WriteNotice("device queue:(" + _cXNARender._aqDisplayQueue.Count + ") baetylus buffer:(" + Baetylus.nAudioBufferCount + ", " + Baetylus.nVideoBufferCount + ") internal buffer:(" + _aq__PROBA__AudioFrames.Count + ", " + _aq__PROBA__VideoFrames.Count + ")");
				}
				catch (Exception ex)
				{
					(new Logger()).WriteError(ex);
	...
	public class XNARender : Microsoft.Xna.Framework.Game
	{
		...
		public void TurnOn(int nWidth, int nHeight)
		{
			...
			//_cGraphicsDeviceManager.IsFullScreen = true;
